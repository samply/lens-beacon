...
export class BeaconResultTableComponent extends ResultRendererComponent {
  public tableData: Array<{site: string, individuals: number, samples: number, diagnosis: number}> = [];

  public selectedSites: Array<{site: string, individuals: number, samples: number, diagnosis: number}> = [];
  ...
  protected override handleUpdatedData(measures: Measure[]) {
    ...
    if (sitesMeasure) {
      this.tableData = sitesMeasure.stratifier
        ...
        .map(site => {
          let variantsStratum = site.stratum.find(strat => strat.key == "variants")
          ...
          let individualsStratum = site.stratum.find(strat => strat.key == "individuals")
          let specimenStratum = site.stratum.find(strat => strat.key == "specimen")
          return {
            site: site.key,
            variants: (variantsStratum != undefined) ? variantsStratum.population : 0,
            ...
            individuals: (individualsStratum != undefined) ? individualsStratum.population : 0,
  ...
  onSelectedSitesChanged($event: Array<{site: string, individuals: number, samples: number, diagnosis: number}>) {
